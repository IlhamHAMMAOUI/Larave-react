...
const CalendrierUpdateComponent = () => {
  const { id } = useParams();
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const eventData = response.data.evenement;
        setFormData({
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      const response = await axios.put(`http://localhost:8000/api/calendrier/update/${id}`, formData);
      console.log('Event updated successfully:', response.data);
      // Rediriger l'utilisateur vers une autre page après la mise à jour de l'événement
...
export default CalendrierUpdateComponent;
